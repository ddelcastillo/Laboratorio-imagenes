...
def MyConnComp_201630945_201622695(binary_image, conn):
    ...
    v = np.sum(binary_image)  # Number of vertexes.
    coord_to_vertex = {}
    # Base UnionFinder
    ...
    parents = np.zeros(v, dtype=numpy.int_)
    for i in range(v):
        parents[i] = root(i)
    parent_to_index = {}
    for index, i in enumerate(set(parents)):
        parent_to_index[i] = index
...
image, labels = MyConnComp_201630945_201622695(test, 8)
